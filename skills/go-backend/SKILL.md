...
- Do not create a function or method that only forwards the same arguments to another helper and returns the result unchanged. Delete or inline pass-through wrappers; if the receiver is unused, prefer a package-level helper or a direct call. When callers need shared behavior with different parsing or validation rules, express the variation through explicit parameters and call the helper directly. Keep a wrapper only when it adds a real contract or ownership boundary such as validation, normalization, defaults, error translation, resource lifecycle, authorization or security, metrics or tracing, or an adapter between independently owned interfaces.
...
- Read `references/cron-expression-parser.md` when implementing or reviewing cron expression parsing, matcher semantics, or next-run time calculation.
