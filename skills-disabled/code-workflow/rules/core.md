...
2. Task-intake analysis gate: relevant analysis skill first, then `grill-with-docs`
3. Coding-start gate
...
9. When a non-trivial task arrives, do not jump straight to plan writing or implementation. First run the relevant analysis skill, then run `grill-with-docs` to challenge gaps, assumptions, owner boundaries, and missing facts. Only after that should the implementation plan be finalized and implementation begin.
10. Default analysis pass is usually `brainstorming`; debugging or failure tasks usually use `systematic-debugging`; if another analysis skill is a better fit, use that instead.
...
- Use `writing-plans` for the implementation plan itself; write the plan in Chinese by default unless the user explicitly requests another language.
- Use the relevant analysis skill at task intake, usually `brainstorming`.
- Use `systematic-debugging` instead when the task starts from a bug, failure, or unexpected behavior.
- Use `grill-with-docs` immediately after the analysis pass, before the plan is considered ready.
...
5. At task intake for a non-trivial task, run the analysis gate in order: relevant analysis skill, then `grill-with-docs`.
